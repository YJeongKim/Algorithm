...
- 재귀적 정의를 이용해서 재귀(순환) 함수를 구현한다.
  ...
  1. 재귀 호출 - 유도 부분 (Inductive part)

  2. 재귀 종료 - 기본 부분 (Base part)
...
함수의 인수 n을 배열 index로 활용하였고, k를 배열의 크기로 하여 재귀 호출의 횟수를 결정짓는 데 활용되었다.

배열의 크기 k와 index n이 같아지면 (n==k) 마무리 작업 후 재귀 호출을 중단한다.
